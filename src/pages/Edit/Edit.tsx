...
export default function Edit() {
	...
	// 处理请求数据
	const dealData = (errCode: Number, msg: String) => {
			...
			.then((values: any) => {
				...
				if (params.id) {
					...
					ArticleUpdateApi({
						...
						content,
						id: params.id,
					}).then((res: any) => dealData(res.errCode, res.message));
				} else {
					// 添加文章的请求
					ArticleAddApi({ title, subTitle, content }).then(
						(res: any) => dealData(res.errCode, res.message)
					);
				}
			})
			.catch(() => false);
	...
	useEffect(() => {
		...
		// 根据地址栏id做请求
		if (params.id) {
			ArticleSearchApi({ id: params.id }).then((res: any) => {
				if (res.errCode === 0) {
	...
	return (
		<div>
			<PageHeader
				...
				title="文章编辑"
				subTitle={
					"当前日期：" + moment(new Date()).format("YYYY-MM-DD")
				}
				extra={
					<Button
						key="1"
